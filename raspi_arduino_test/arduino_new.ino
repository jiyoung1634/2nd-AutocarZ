...
// 목표 속도(라즈베리파이 명령어)와 실제 속도(모터에 지금 적용된 값)
uint8_t targetL = 0, targetR = 0;  // 목표 속도(0~255)
uint8_t currL = 0, currR = 0;  // 현재(실제) 속도(0~255)

// 속도를 얼마나 자주/얼마나 조금씩 바꿀지
const uint8_t SLEW_STEP = 8;  // 한번에 바꿀 수 있는 최대 속도 변화(작을수록 더 부드러움)
const uint16_t SLEW_DT_MS = 20;  // 이 간격(ms)마다 조금씩 바꿈 (20ms = 0.02초)
uint32_t lastSlewUpdate = 0;

// 목표 속도만 정하는 함수
void setTargetSpeeds(uint8_t l, uint8_t r) {
  targetL = constrain(l, 0, 255);
  targetR = constrain(r, 0, 255);
  leftMotor.run(FORWARD);
  rightMotor.run(FORWARD);
}

// 현재 속도를 목표 속도로 살살 따라가게 하는 함수
void applySlew() {
  if (millis() - lastSlewUpdate < SLEW_DT_MS) return;
  lastSlewUpdate = millis();

  int dL = (int)targetL - (int)currL;  // 목표-현재 차이
  if (dL > 0) currL += min((int)SLEW_STEP, dL);  // 너무 뛰지 말고 SLEW_STEP만큼만
  else if (df < 0) currL -= min((int)SLEW_STEP, -dL);

  int dR = (int)targetR - (int)currR;
  if (dR > 0) currR += min((int)SLEW_STEP, dR);
  else if (dR < 0) currR -= min((int)SLEW_STEP, -dR);

  leftMotor.setSpeed(currL);
  rightMotor.setSpeed(currR);
}
...
// --- 앞으로(양쪽 전진, 같은 속도)
void forward() {
  // setSpeed않고, "목표 속도"만 ㅓㄹ정 -> applySlew()가 천천히 따라감
  setTargetSpeeds(BASE_SPEED, BASE_SPEED);
}
...
void loop() {
  ...
  // 매 20ms마다 "현재 속도"를 "목표 속도"로 살살 따라가게
  applySlew()
}
