...
# 스무딩(EMA) 파라미터
SMOOTH_BETA = 0.25  # 0~1 (작을수록 더 부드러움)
smoothed_x = None  # 처음에는 아직 없음 표시
...
def extract_white_mask(bgr_roi):
    ...
    # 너무 밝은 값(v)을 살짝 눌러서(clip) 형광등 번쩍임 영향 약화
    v = hsv[:, :, 2]
    np.minimum(v, 240, out=v)  # 255였던 아주 밝은 픽셀을 240으로 잘라냄
...
# ④ 제어 파라미터(필요시 조정)
DEADBAND = 30          # 가운데에서 좌우 ±20픽셀 이내면 "직진(F)"로 간주
...
def decide_command(mask_width, peak_x, center_x): # 자동차가 어디로 가야 할지 결정하는 함수   
    ...
    반환: 'F','L','R','S' 중 하나
    히스테리시스 적용 → L/R 상태였다가 F로 돌아오려면 더 중앙으로 와야 함
    """
    ...
    # 히스테리시스 : L/R에서 F로 돌아올 때는 band를 더 좁게 해서 너무 자주 안바뀌게 함
    go_band = DEADBAND  # 일반적으로 F로 갈 때 기준
    return_band = DEADBAND - 8  # L/R → F로 "복귀"할 땐 더 중앙이어야 함(조금 더 엄격)
    band = return_band if last_cmd in ('L', 'R') else go_band
...
while True:
    ...
    if total_white > NOISE_MIN:
        ...
        left_peak_x = int(left_hist.argmax())
        # 오른쪽 위치는 center_x를 더해주어야 전체 화면 기준 좌표가 됩니다.
        right_peak_x = int(right_hist.argmax()) + center_x
        # 두 차선 위치의 평균을 내어 도로의 중앙을 계산
        lane_center_x = (left_peak_x + right_peak_x) // 2

        # 스무딩(EMA) 적용 -> 갑자기 튀는 걸 줄임
        if smoothed_x is None:
            smoothed_x = lane_cener_x
        else:
            smoothed_x = int((1 - SMOOTH_BETA) * smoothed_x + SMOOTH_BETA * lane_center_x)
        peak_X = smoothed_x
